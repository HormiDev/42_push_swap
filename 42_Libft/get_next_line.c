...
t_buffer_lst	*ft_addnewlst(t_buffer_lst *lst)
{
	...
	if (node->content == 0)
	{
		ft_lstclear_gnl(&lst, free);
		ft_lstclear_gnl(&node, free);
		return (0);
	}
...
char	*get_next_line_2(int fd, t_buffer_lst **buffer)
{
	...
	while (ft_strnchr(ft_lstlast_gnl(*buffer)->content, '\n',
			ft_lstlast_gnl(*buffer)->lencontent) == -1)
	{
		*buffer = ft_addnewlst(*buffer);
		temp = ft_lstlast_gnl(*buffer);
		temp->lencontent = read(fd, temp->content, BUFFER_SIZE);
		if (ft_lstlast_gnl(*buffer)->lencontent <= 0)
		{
			line = lstjoin(*buffer, ft_lstlast_gnl(*buffer)->lencontent - 1);
			ft_lstclear_gnl(buffer, free);
			return (line);
		}
	}
	endline = ft_strnchr(ft_lstlast_gnl(
				*buffer)->content, '\n', ft_lstlast_gnl(*buffer)->lencontent);
...
int main()
{
	...
	printf("%d\n", ft_lstsize(lst));
	char *line = lstjoin(lst, ft_strnchr(ft_lstlast_gnl(lst)->content, '\n',
	 BUFFER_SIZE));
	printf("%s", line);
	lst = cleanbuffer(lst, ft_strnchr(ft_lstlast_gnl(lst)->content, '\n',
	 BUFFER_SIZE));
	printf("%d\n", lst->lencontent);
	cont = 0;
...
char	*get_next_line(int fd)
{
	...
	{
		buffer = ft_addnewlst(buffer);
		ft_lstlast_gnl(buffer)->lencontent = read(fd,
		 ft_lstlast_gnl(buffer)->content, BUFFER_SIZE);
		if (ft_lstlast_gnl(buffer)->lencontent <= 0)
		{
			line = lstjoin(buffer, ft_lstlast_gnl(buffer)->lencontent - 1);
			ft_lstclear_gnl(&buffer, free);
	...
	endline = ft_strnchr(ft_lstlast_gnl(buffer)->content, '\n',
	 ft_lstlast_gnl(buffer)->lencontent);
	line = lstjoin(buffer, endline);
	if (line == 0)
		return (0);
